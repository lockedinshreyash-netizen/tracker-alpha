...
- The editor asks **what it is first**, and only then shows the fields that kind actually has — subject and chapter for study, a free-text name for everything else. Switching families clears the detail that no longer applies, so a block cannot keep a subject it stopped having.
...
- `schedule/colors.ts` is the app's only subject colour map; it did not exist before this feature. The accent `#E10600` is deliberately absent from both palettes — that belongs to the now-line and to actions.
...
- **Tabs** (`Today`, `Plan`, `Syllabus`, `Streak`, `Questions`, `Ranks`, `Review`) map to the app's core loops:
  - `Plan` — the whole day on one timeline: study alongside class, meals, the gym and sleep. Drag to move, drag an edge to resize, tap to edit, one-tap quick-add per activity, a Repeat control (just today / every day / every weekday) on each block, and Start now to put the clock on a study block. Plus a repeating-blocks section and a plan-vs-actual adherence readout that grades study only. See Scheduling above.
