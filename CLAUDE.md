...
- **Tabs** (`Today`, `Plan`, `Syllabus`, `Streak`, `Questions`, `Ranks`, `Review`) map to the app's core loops:
  - `Plan` — the whole day on one timeline: study alongside class, meals, the gym and sleep. Drag to move, drag an edge to resize, tap to edit, one-tap quick-add per activity, ENGAGE to start the clock on a study block. Plus a weekly recurring template and a plan-vs-actual adherence readout that grades study only. See Scheduling above.
